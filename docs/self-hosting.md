...
| `TOMBSTONE_RETENTION_DAYS` | `30`     | 回收站保留天数：软删除墓碑到期由每日任务硬删；客户端 UI 的「剩余 N 天」取自该值 |
| `PORT`                     | `3000`   | 监听端口                                                                        |
...
- **「仅本地」** 标记的条目永不离开浏览器；已同步的条目打开此开关会先从服务器删除（服务端只留下墓碑，本机条目仍在片段库，也不会因此出现在本机回收站）。

## 回收站

删除不再是不可逆操作：条目先变成墓碑（`Snippet.deletedAt`）在回收站保留 `TOMBSTONE_RETENTION_DAYS` 天，期间客户端可恢复或彻底删除，到期由每日任务物理删除。相关端点（均需登录）：

| 方法与路径                       | 说明                                                                    |
| -------------------------------- | ----------------------------------------------------------------------- |
| `GET /api/snippets/trash`        | 列出墓碑（按删除时间倒序），`meta.retentionDays` 回传保留天数           |
| `POST /api/snippets/:id/restore` | 恢复：清墓碑并同时推进 `updatedAt` / `syncedAt`，其它设备同步后也会恢复 |
