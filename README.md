...
- **手动保存与「已保存」页面**：没有任何自动保存——只有点工具栏「保存」或按 `Ctrl/Cmd+S` 才把当前内容存进片段库，状态栏实时显示「未保存 / 已保存」，复制不会顺带入库。工具栏「已保存」打开独立的片段库页面：条目取首行作为标题，按「今天 / 昨天 / 7 天内 / 30 天内 / 更早」分组，支持搜索、类型筛选、置顶、恢复到编辑器、删除单条与二次确认清空全部；点击条目进入**详情页**，展示完整信息（类型、语言、字符/行/字数、创建/更新时间、标签、收藏夹、同步状态）与逐字全文。未保存的修改在打开其他条目前会弹确认（保存并继续 / 不保存）。仅保存在本浏览器 localStorage，不上传；最多 30 条、单条上限 10 万字符。
- **颜色主题**：深色（默认）/ 浅色 / 高对比，工具栏或设置面板中均可切换。
- **隐私**：无后端、无账号、无分析统计；编辑内容只在浏览器本地处理、不写入 URL、IndexedDB、Service Worker 缓存或日志。唯一的本地持久化是偏好设置与手动保存的片段库（不保存就什么都不留）；生产构建注入 CSP（仅限同源资源）。
...
npm run test:e2e      # Playwright 端到端（K3s 核心流程、Vim 操作、手动保存与片段库、Prompt 片段、桌面/移动视口、离线）
...
- **自托管登录版**（Docker 镜像 / k3s）：在匿名版之上提供**账号 + 云端片段库**——登录后片段同步到你自己的服务器，支持跨设备、搜索、收藏夹、标签、置顶——每个账号自带一个 `default` 收藏夹，保存新片段时可在「新片段」栏选定落点，默认进 `default`；新增 **Prompt 类型**片段（软换行、`{{变量}}` 占位符、变量填充并一键复制给 LLM）；未登录时与匿名版行为完全一致。
  - **收藏夹管理**：片段库顶部是收藏夹面板，每个收藏夹带颜色圆点与条目计数，点名称即筛选；点铅笔打开管理对话框，可改名称、从调色板换颜色、上移/下移排序、删除。删除前会二次确认并告诉你会影响多少条片段（片段不会被删掉，只会变成「未分类」）。`default` 是系统收藏夹（新片段的默认落点），不能重命名或删除，但可以换颜色与调整顺序。
  - **把片段移进收藏夹**：片段库每一行都有文件夹图标，点开即可选收藏夹（含「未分类」与「＋ 新建收藏夹…」现建现用）；详情页的「收藏夹」是可编辑下拉；编辑器里的「编辑中」条目栏也保留同一入口。三条路径共用同一套写回逻辑，改动会进入同步队列。
...
- **手动保存的片段库**：编辑内容只在你点「保存」（Ctrl/Cmd+S 或工具栏按钮）时写入本浏览器 localStorage（键 `vimpaste.history.v1`）；没有任何自动保存，复制也不会入库。「已保存」页面可查看详情、搜索、删除单条或清空全部。
- 未保存的内容只存在于当前标签页内存中，刷新或关闭页面后立即消失；请留意状态栏的「未保存」提示。
- 另在 localStorage 保存非敏感偏好：键位模式、字体大小、首次提示是否已关闭、颜色主题。
...
| `Ctrl/Cmd+S`     | 保存当前内容到片段库（唯一的保存入口，没有自动保存）                |
| `Ctrl/Cmd+F`     | 打开搜索面板                                                        |
