...
- **自托管登录版**（Docker 镜像 / k3s）：在匿名版之上提供**账号 + 云端片段库**——登录后片段同步到你自己的服务器，支持跨设备、搜索、收藏夹、标签、置顶——每个账号自带一个 `default` 收藏夹，保存新片段时可在「新片段」栏选定落点，默认进 `default`；新增 **Prompt 类型**片段（软换行、`{{变量}}` 占位符、变量填充并一键复制给 LLM）；未登录时与匿名版行为完全一致。
  ...
  - **回收站（登录版同样适用）**：删除进回收站保留 30 天（`TOMBSTONE_RETENTION_DAYS`，删除先写服务端墓碑、到期由服务端每日任务硬删），期间可恢复或彻底删除；「彻底删除」与「清空回收站」在登录版下会请服务端立即物理删除，不等 30 天。跨设备一致：在 A 机删除，B 机同步后也进 B 机的回收站；在 A 机恢复，B 机同步后条目回到片段库。服务端回收站列表在进入回收站页面时拉取，离线新建后立刻删除的条目也仍能在本机看到（标「未同步」）。
