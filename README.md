...
- **回收站（30 天内可恢复）**：删除条目不再立即消失，而是进入回收站——片段库页脚的「回收站」入口（带条数角标）打开 `#/trash`，按「今天 / 昨天 / 7 天内 / 30 天内」分组列出（与片段库同一套分组规则，只是按删除时间），每行显示删除时间与**剩余天数**，行内可「恢复」回片段库或「彻底删除」，页脚可二次确认「清空回收站」。保留期 30 天，到期自动清除：本地路径在打开应用时惰性清理（并把过期墓碑从 localStorage 真正删掉），登录路径另由服务端的每日任务硬删（`TOMBSTONE_RETENTION_DAYS`，自托管可改，界面显示的天数取自服务端配置）。删除收藏夹不产生回收站条目（片段只会变为「未分类」）；「仅本地」条目在服务端留下的墓碑只是其它设备的删除信号，不会在自己机器的回收站里出现。
...
npm run test:e2e      # Playwright 端到端（K3s 核心流程、Vim 操作、手动保存与片段库、回收站、Prompt 片段、桌面/移动视口、离线）
...
- **自托管登录版**（Docker 镜像 / k3s）：在匿名版之上提供**账号 + 云端片段库**——登录后片段同步到你自己的服务器，支持跨设备、搜索、收藏夹、标签、置顶——每个账号自带一个 `default` 收藏夹，保存新片段时可在「新片段」栏选定落点，默认进 `default`；新增 **Prompt 类型**片段（软换行、`{{变量}}` 占位符、变量填充并一键复制给 LLM）；未登录时与匿名版行为完全一致。
  ...
  - **回收站（登录版同样适用）**：删除进回收站保留 30 天（`TOMBSTONE_RETENTION_DAYS`，删除先写服务端墓碑、到期由服务端每日任务硬删），期间可恢复或彻底删除；「彻底删除」与「清空回收站」在登录版下会请服务端立即物理删除，不等 30 天。跨设备一致：在 A 机删除，B 机同步后也进 B 机的回收站；在 A 机恢复，B 机同步后条目回到片段库。服务端回收站列表在进入回收站页面时拉取，离线新建后立刻删除的条目也仍能在本机看到（标「未同步」）。注意：「彻底删除」与「清空回收站」是**立即**物理删除、不等墓碑传到其它设备；若某台设备在同步到这条删除前仍留着活副本，之后在那台设备上编辑它会把它当作新建重新上传——确认各常用设备已同步后再彻底删除。
...
- **手动保存的片段库**：编辑内容只在你点「保存」（Ctrl/Cmd+S 或工具栏按钮）时写入本浏览器 localStorage（键 `vimpaste.history.v1`）；没有任何自动保存，复制也不会入库。「已保存」页面可查看详情、搜索、删除单条或清空全部；删除后的条目进回收站保留 30 天，可在回收站里恢复或彻底删除。
