...
- **手动保存与「已保存」页面**：没有任何自动保存——只有点工具栏「保存」或按 `Ctrl/Cmd+S` 才把当前内容存进片段库，状态栏实时显示「未保存 / 已保存」，复制不会顺带入库。工具栏「已保存」打开独立的片段库页面：条目取首行作为标题，按「今天 / 昨天 / 7 天内 / 30 天内 / 更早」分组，支持搜索、类型筛选、置顶、恢复到编辑器、删除单条与二次确认清空全部；点击条目进入**详情页**，展示完整信息（类型、语言、字符/行/字数、创建/更新时间、标签、集合、同步状态）与逐字全文。未保存的修改在打开其他条目前会弹确认（保存并继续 / 不保存）。仅保存在本浏览器 localStorage，不上传；最多 30 条、单条上限 10 万字符。
